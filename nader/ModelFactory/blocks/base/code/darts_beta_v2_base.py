...
@Registers.block
class darts_beta_v2_base(nn.Module):
    ...
    def __init__(self,in_channels,out_channels,C_pre=None,reduction_pre=False):
        ...
        self.op17=ReLU()
        self.op18=CustomConv2d(in_channels=in_channels,out_channels=out_channels,kernel_size=5,stride=1,dilation=1,groups=out_channels)
        ...
        self.op21=ReLU()
        self.op22=CustomConv2d(in_channels=out_channels,out_channels=out_channels,kernel_size=5,stride=1,dilation=1,groups=out_channels)
        ...
        self.op25=ReLU()
        self.op26=CustomConv2d(in_channels=out_channels,out_channels=out_channels,kernel_size=3,stride=1,dilation=1,groups=out_channels)
        ...
        self.op33=ReLU()
        self.op34=CustomConv2d(in_channels=out_channels,out_channels=out_channels,kernel_size=3,stride=1,dilation=2,groups=out_channels)
        ...
        self.op37=ReLU()
        self.op38=CustomConv2d(in_channels=(((out_channels)+(out_channels))+(out_channels))+(out_channels),out_channels=out_channels,kernel_size=1,stride=1,dilation=1,groups=1)
        self.op39=BatchNorm2d(num_features=out_channels)
    ...
    def forward(self,x1,x2=None,drop_path_prob=0.0):
        ...
        h4 = h4+x2
        h6 = torch.concat([h1,h2,h4,h5], dim=1)
        h6 = self.op37(h6)
        h6 = self.op38(h6)
        h6 = self.op39(h6)
        return h6

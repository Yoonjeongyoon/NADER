...
@Registers.model
class dartsv2_seed777_dartsv2_p2930(nn.Module):

    def __init__(self,num_classes=100,auxiliary=False) -> None:
        super().__init__()
        self.auxiliary = auxiliary
        base = Registers.block['dartsv2_seed777_dartsv2_p2930_base']
        stem = Registers.block['dartsv2_stem']
        downsample = Registers.block['dartsv2_downsample']
        self.stem = stem(3,33)
        cells_type = [0]*5+[1]+[0]*5+[1]+[0]*8
        cell_widths = [33]*5+[66]+[66]*5+[132]+[132]*8
        self.layer_num = sum([5, 5, 8])+2
        self.cells = nn.ModuleList()
        reduction_pre,reduction_pre_pre = False, False
        width_pre,width_pre_pre = 33,33
        ...
        if auxiliary:
            self.auxiliary_head = AuxiliaryHeadCIFAR(132, num_classes)

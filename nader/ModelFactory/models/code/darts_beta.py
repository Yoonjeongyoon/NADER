...
@Registers.model
class darts_beta(nn.Module):

    def __init__(self,num_classes=100,auxiliary=False) -> None:
        super().__init__()
        self.auxiliary = auxiliary
        base = Registers.block['darts_beta_base']
        stem = Registers.block['darts_beta_stem']
        downsample = Registers.block['darts_beta_downsample']
        self.stem = stem(3,36)
        cells_type = [0]*5+[1]+[0]*5+[1]+[0]*8
        cell_widths = [36]*5+[72]+[72]*5+[144]+[144]*8

...
@Registers.model
class nasbench201_cifar100_acc76_forRes50(nn.Module):

    def __init__(self,num_classes=100) -> None:
        super().__init__()
        block = Registers.block['nasbench201_cifar100_acc76_forRes50_base']
        ...
        self.stem = stem(in_channels=3,out_channels=50)
        layers = []
        for _ in range(3):
            layers.append(block(in_channels=50,out_channels=50))
        self.layer1 = nn.Sequential(*layers)
        self.downsample1 = downsample(in_channels=50,out_channels=100)
        layers = []
        for _ in range(3):
            layers.append(block(in_channels=100,out_channels=100))
        self.layer2 = nn.Sequential(*layers)
        self.downsample2 = downsample(in_channels=100,out_channels=200)
        layers = []
        for _ in range(9):
            layers.append(block(in_channels=200,out_channels=200))
        self.layer3 = nn.Sequential(*layers)
        self.downsample3 = downsample(in_channels=200,out_channels=400)
        layers = []
        for _ in range(3):

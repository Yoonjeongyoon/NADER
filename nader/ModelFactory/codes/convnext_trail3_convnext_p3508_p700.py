...
@Registers.model
class convnext_trail3_convnext_p3508_p700(nn.Module):
    ...
    def __init__(self,num_classes=100) -> None:
        ...
        dp_rates=[x.item() for x in torch.linspace(0, 0.2, 18)] 
        self.stem = stem(in_channels=3,out_channels=92)
        layers = []
        for i in range(3):
            layers.append(block(in_channels=92,out_channels=92,drop_path=dp_rates[i]))
        self.layer1 = nn.Sequential(*layers)
        self.downsample1 = downsample(in_channels=92,out_channels=184)
        layers = []
        for i in range(3):
            layers.append(block(in_channels=184,out_channels=184,drop_path=dp_rates[3+i]))
        self.layer2 = nn.Sequential(*layers)
        self.downsample2 = downsample(in_channels=184,out_channels=368)
        layers = []
        for i in range(9):
            layers.append(block(in_channels=368,out_channels=368,drop_path=dp_rates[6+i]))
        self.layer3 = nn.Sequential(*layers)
        self.downsample3 = downsample(in_channels=368,out_channels=736)
        layers = []
        for i in range(3):
            layers.append(block(in_channels=736,out_channels=736,drop_path=dp_rates[15+i]))

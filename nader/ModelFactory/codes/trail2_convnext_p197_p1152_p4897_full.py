...
class convnext_p197_p1152_p4897_base(nn.Module):
    ...
    def __init__(self,in_channels,out_channels=None):
        ...
        self.op6=CustomConv2d(in_channels=(in_channels)//(1),out_channels=in_channels,kernel_size=3,stride=1,dilation=1,groups=in_channels)
        self.op7=CustomConv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=3,stride=1,dilation=1,groups=in_channels)
        self.op8=CustomConv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=3,stride=1,dilation=1,groups=in_channels)
        self.op9=CustomConv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=3,stride=1,dilation=1,groups=in_channels)
...
class convnext_p197_p1152_p4897_base2(nn.Module):
    ...
    def forward(self,x):
        ...
        h1 = self.op6(h1)
        h1 = h1+h2+h3
        h1 = self.drop_path(h1)+x
        return h1

class convnext_p4265_stem(nn.Module):
...
class convnext_p599_downsample(nn.Module):
...
@Registers.model
class trail2_convnext_p197_p1152_p4897_full(nn.Module):

    def __init__(self,num_classes=100) -> None:
        super().__init__()
        block = convnext_p197_p1152_p4897_base
        stem = convnext_p4265_stem
        downsample = convnext_p599_downsample
        dim = 98
        ...
        layers = []
        for _ in range(3):
            layers.append(block(in_channels=dim*8,out_channels=dim*8))
...
@Registers.model
class trail2_convnext_p197_p1152_p4897_full2_3393(nn.Module):

    def __init__(self,num_classes=100) -> None:
        super().__init__()
        block = convnext_p197_p1152_p4897_base2
        stem = convnext_p4265_stem
        downsample = convnext_p599_downsample
        dim = 98
        dp_rates=[x.item() for x in torch.linspace(0, 0.1, 18)] 
        self.stem = stem(in_channels=3,out_channels=dim)
        layers = []
        for i in range(3):
        ...
        layers = []
        for i in range(3):
            layers.append(block(in_channels=dim*2,out_channels=dim*2,drop_path=dp_rates[3+i]))
        self.layer2 = nn.Sequential(*layers)
        self.downsample2 = downsample(in_channels=dim*2,out_channels=dim*4)
        layers = []
        for i in range(9):
            layers.append(block(in_channels=dim*4,out_channels=dim*4,drop_path=dp_rates[6+i]))
        self.layer3 = nn.Sequential(*layers)
        self.downsample3 = downsample(in_channels=dim*4,out_channels=dim*8)
        layers = []
        for i in range(3):
            layers.append(block(in_channels=dim*8,out_channels=dim*8,drop_path=dp_rates[15+i]))
        self.layer4 = nn.Sequential(*layers)
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(dim*8,num_classes)
